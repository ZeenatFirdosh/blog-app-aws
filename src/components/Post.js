...
function Post() {
  ...
  const fetchPosts = async () => {
    try {
      const response = await axios.get('https://qbw3paee82.execute-api.ap-south-1.amazonaws.com/dev/blog');
      ...
      <div class="min-h-28 ">
        <div class="max-w-screen-lg mx-auto py-4">
          ...
          {/* <nav class="flex gap-3 flex-wrap justify-center text-lg">
            <button
              class="text-center my-2 inline-block w-40 rounded-full bg-blue-800 bg-opacity-10 px-4 py-2 font-semibold text-blue-800 duration-200 hover:bg-opacity-95 hover:text-white hover:no-underline sm:w-48"
                      ...
                      <div className="flex flex-wrap gap-1 mb-1 justify-end">
                        {item.categories &&
                          ...
                          item.categories.map((c) => (
                            <span class="bg-green-100 text-green-800 text-[8px] font-medium me-2 px-2.5 py-0.5 rounded-full dark:bg-green-900 dark:text-green-300">
                              {c.title}
                            </span>
                          ))}
                      </div>
